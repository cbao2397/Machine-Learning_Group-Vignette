...
The objective of our machine learning module is to predict outcomes with accuracy that produce data-driven insights.  
...
Transfer learning is a machine learning technique where a model trained on one task is adapted for a second related task. Instead of training a model from scratch for each specific task, a machine exploits the knowledge gained from a previous task to improve generalization about another. 
...
Now you will try Supervised Machine Learning on your own using the Breast Cancer Wisconsin (Diagnostic). This dataset characterizes features of cell nuclei from digitized images of fine needle aspirates (FNA) of breast masses. Using machine learning for this dataset, we will train the model to recognize features of the FNAs so we can efficiently and accurately predict whether a patient has malignant or benign breast cancer. The dataset includes the following variables:  

-ID number  
-Diagnosis (0=malignant, 1=benign)  
-radius1, radius2, radius3 (mean of distances from center to points on the perimeter)  
-texture1, texture2, texture3 (standard deviation of gray-scale values)  
-perimeter1, perimeter2, perimeter3  
-area1, area2, area3
-smoothness1, smoothness2, smoothness3 (local variation in radius lengths)  
-compactness1, compactness2, compactness3 (perimeter^2/area-1.0)   
-concavity1, concavity2, concavity3 (severity of concave portions of the contous)   
-concave_points1, concave_points2, concave_points3 (number of concave portions of the contour)   
-symmetry1, symmetry2, symmetry3   
-fractal_dimension1, fractal_dimension2, fractal_dimension3 ("coastline approximation" -1)
...
Don't forget to graph the ROC curve. This model is even more accurate than our first one! It has an AUC value of **0.9648043**. 
