...
plot4 <- ggplot(data = model4, aes(x = age + sex + cp + trestbps + chol + restecg + thalach + exang + oldpeak + slope + ca + thal + num, y = fbs))
...
**note:move to evaluating the model**
...
summary(p)
cl <- ifelse(p > 0.15, "1", "0")
t <- table(cl, test_2$fbs)
confusionMatrix(t)
```

**L: **Oh my god the accuracy looks horrible though it actually looks better than what they have in the example. 
...
```{r}
library(caTools)
caTools::colAUC(p, test_2[["fbs"]], plotROC = TRUE)
```
The returned value represents the area under the curve (AUC) model accuracy metric. We have value as approximately 0.6 which is not too bad (at least better than a random guess). Hope we can improve that to be closer to 1, which represents a perfect model. 